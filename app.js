...
function bindStaticEvents() {
  ...
  document.getElementById("removeCountryButton").addEventListener("click", removeSelectedCountry);

  document.querySelectorAll("[data-country-status]").forEach(button => {
    button.addEventListener("click", () => setCountryStatus(button.dataset.countryStatus));
  });

  document.querySelectorAll("[data-country-color]").forEach(button => {
    button.addEventListener("click", () => {
      document.getElementById("countryColor").value = button.dataset.countryColor;
    });
  });
  ...
  document.getElementById("closeMemoryAlbumButton").addEventListener("click", closeMemoryAlbum);
  document.getElementById("memoryAlbumModal").addEventListener("click", event => {
    if (event.target.id === "memoryAlbumModal") closeMemoryAlbum();
  });
...
async function loadMemories() {
  ...
  memoryAlbumCache = data || [];

  const cards = await Promise.all(memoryAlbumCache.map(async memory => {
    ...
    if (firstPhoto) {
      ...
      if (signed?.signedUrl) {
        cover = `<img src="${signed.signedUrl}" alt="Memory cover">`;
      }
    }

    return `
      <article class="memory-card" onclick="openMemoryAlbum('${memory.id}')">
        <div class="memory-cover">
          ${cover}
          <span class="memory-photo-count">📷 ${photoCount}</span>
        </div>
        ...
        <div class="card-body">
          ...
          <div class="card-meta">
            ...
            ${memory.location ? ` · ${escapeHtml(memory.location)}` : ""}
          </div>

          <p>${escapeHtml(memory.description || "")}</p>

          ${memory.favorite_moment
            ? `<p><strong>Favorite moment:</strong> ${escapeHtml(memory.favorite_moment)}</p>`
            : ""}

          <div class="memory-open-hint">Open album →</div>
          ...
          <div class="card-actions" onclick="event.stopPropagation()">
            <label class="small-action">
              ...
              <input
                ...
                accept="image/*"
                multiple
                onchange="uploadMultipleMemoryPhotos('${memory.id}',this)"
              >
            </label>

            <button
              class="small-action danger"
              onclick="deleteRow('memories','${memory.id}',loadMemories)">
              Delete
            </button>
          </div>
        </div>
  ...
  document.getElementById("memoryGrid").innerHTML =
    cards.join("") || emptyState("No memories yet.");
}

async function openMemoryAlbum(memoryId) {
  let memory = memoryAlbumCache.find(item => item.id === memoryId);
  ...
  document.getElementById("memoryAlbumContent").innerHTML = `
    <div class="album-header">
      <div>
        ...
        ${memory.description
          ? `<p class="album-description">${escapeHtml(memory.description)}</p>`
          : ""}

        ${memory.favorite_moment
          ? `<div class="album-favorite"><strong>Favorite moment:</strong> ${escapeHtml(memory.favorite_moment)}</div>`
          : ""}
      </div>
    </div>
    ...
    <div class="album-toolbar">
      <label class="album-upload-button">
        ...
        <input
          ...
          accept="image/*"
          multiple
          onchange="uploadMultipleMemoryPhotos('${memory.id}',this,true)"
        >
      </label>
      <span class="album-count">${validPhotos.length} photo${validPhotos.length === 1 ? "" : "s"}</span>
    </div>
    ...
    <div class="album-photo-grid">
      ${validPhotos.length
        ? validPhotos.map(photo => `
          ...
          `).join("")
        : `<div class="album-empty">No photos in this album yet. Add your first one 📸</div>`
      }
    </div>
  `;

  document.getElementById("memoryAlbumModal").classList.remove("hidden");
}

function closeMemoryAlbum() {
  document.getElementById("memoryAlbumModal").classList.add("hidden");
  document.getElementById("memoryAlbumContent").innerHTML = "";
}

async function uploadMultipleMemoryPhotos(memoryId, input, reopenAlbum = false) {
  ...
  try {
    ...
    if (reopenAlbum) {
      await openMemoryAlbum(memoryId);
    }
  } catch (error) {
    alert(error.message);
  }
...
function openMemoryModal() {
  openModal(`
    <form id="memoryForm" class="modal-form">
      ...
      <input id="memoryPhoto" type="file" accept="image/*" multiple>
  ...
  document.getElementById("memoryForm").addEventListener("submit", async event => {
    ...
    const files = Array.from(document.getElementById("memoryPhoto").files || []);
    for (const file of files) {
      await saveMemoryPhoto(data.id, file);
    }
...
async function uploadMemoryPhoto(memoryId, input) {
  await uploadMultipleMemoryPhotos(memoryId, input);
}
...
async function loadCountries() {
  ...
  document.getElementById("countryList").innerHTML =
    countries.map(country => {
      const color = country.custom_color || defaultCountryColor(country.status);
      return `
        <div class="country-v8-item">
          <span class="country-v8-color" style="--country-color:${escapeHtml(color)}"></span>
          <div class="country-v8-name">
            <strong>${countryFlag(country.country_code)} ${escapeHtml(country.country_name)}</strong>
            <small>${escapeHtml(country.status)}${country.notes ? ` · ${escapeHtml(country.notes)}` : ""}</small>
          </div>
          <button type="button" class="country-v8-edit"
            onclick="selectCountry('${country.country_code}','${escapeAttribute(country.country_name)}')">Edit</button>
        </div>
      `;
    }).join("") || `<div class="check-meta">No colored countries yet.</div>`;
}
...
function rebuildWorldMap() {
  ...
  if (typeof window.jsVectorMap === "undefined") {
    container.innerHTML = `<div class="map-loading-message map-error-message">The map library could not load. Refresh the page once.</div>`;
    return;
  }
  ...
  const scale = { neutral: "#b8a8bd" };
  const values = {};

  countries.forEach(country => {
    const color = country.custom_color || defaultCountryColor(country.status);
    const key = `country_${country.country_code}`;
    scale[key] = color;
    values[country.country_code] = key;
  });
  ...
  try {
    ...
    container.innerHTML = `<div class="map-loading-message map-error-message">The map could not start. Refresh and open World Map again.</div>`;
  }
}
...
function resizeWorldMap() {
  ...
  setTimeout(() => {
    try {
      if (typeof worldMap.updateSize === "function") worldMap.updateSize();
      window.dispatchEvent(new Event("resize"));
    } catch (_error) {}
  }, 120);
}

function defaultCountryColor(status) {
  if (status === "Visited") return "#c985a7";
  if (status === "Want to visit") return "#947db0";
  if (status === "Dream destination") return "#79a58f";
  return "#b8a8bd";
}

function setCountryStatus(status) {
  document.getElementById("countryStatus").value = status;
  document.querySelectorAll("[data-country-status]").forEach(button => {
    button.classList.toggle("active", button.dataset.countryStatus === status);
  });

  const existing = countries.find(country => country.country_code === value("countryCode"));
  if (!existing?.custom_color) {
    document.getElementById("countryColor").value = defaultCountryColor(status);
  }
}
...
function selectCountry(code, name) {
  ...
  const displayName = existing?.country_name || name || code;

  document.getElementById("countryCode").value = code;
  document.getElementById("countryName").value = displayName;
  document.getElementById("countryNotes").value = existing?.notes || "";
  document.getElementById("countryColor").value = existing?.custom_color || defaultCountryColor(status);
  document.getElementById("countryEditorTitle").textContent = `${countryFlag(code)} ${displayName}`;
  document.getElementById("selectedCountryHint").textContent = existing ? "Already in your map — edit it below" : "New country — choose how you want to color it";
  setCountryStatus(status);

  if (window.innerWidth < 950) {
    document.querySelector(".country-editor-v8")?.scrollIntoView({ behavior: "smooth", block: "start" });
  }
}

async function saveCountry() {
  const code = value("countryCode");
  if (!code) return alert("Click a country on the map first.");

  const status = value("countryStatus");
  const customColor = document.getElementById("countryColor").value;
...
async function removeSelectedCountry() {
  const code = value("countryCode");
  if (!code) return alert("Click a country first.");
  if (!confirm("Remove this country from your colored map?")) return;
  ...
  document.getElementById("countryNotes").value = "";
  document.getElementById("countryEditorTitle").textContent = "Add or edit a country";
  document.getElementById("selectedCountryHint").textContent = "Click a country on the map";
  setCountryStatus("Visited");
  await loadCountries();
}
...
window.uploadMultipleMemoryPhotos = uploadMultipleMemoryPhotos;
window.openMemoryAlbum = openMemoryAlbum;
window.openLightbox = openLightbox;
window.selectCountry = selectCountry;
window.removeSelectedCountry = removeSelectedCountry;

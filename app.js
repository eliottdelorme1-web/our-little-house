...
const pageInfo = {
  home:["Home","Welcome back to your little shared world."],
  todo:["To-Do List","Shared tasks that are easy to add and complete."],
  grocery:["Grocery List","Everything you still need to buy."],
  dreams:["Dreams & Ideas","Your wishes and shared ideas in one place."],
  ...
  food:["Restaurants","Visited restaurants and places to try."],
...
function bindButtons(){
  ...
  document.querySelectorAll("#entertainmentFilters .category-tab").forEach(button => {
    button.addEventListener("click", () => {
      entertainmentFilter = button.dataset.filter;
      document.querySelectorAll("#entertainmentFilters .category-tab")
...
async function loadDashboard(){
  const [todo,grocery,memory,restaurant]=await Promise.all([
  ...
  document.getElementById("todoCount").textContent=todo.count||0;
  document.getElementById("groceryCount").textContent=grocery.count||0;
  document.getElementById("memoryCount").textContent=memory.count||0;
  document.getElementById("restaurantCount").textContent=restaurant.count||0;
}

async function loadPage(page){
  if(page==="home") return loadDashboard();
  if(page==="todo") return loadTodos();
  if(page==="grocery") return loadGroceries();
...
async function addTodo(){
  try{
    const title=requireText("todoTitle","Task");
...
async function loadTodos(){
  ...
  document.getElementById("todoList").innerHTML=(data||[]).map(item=>`
    <div class="check-item ${item.completed ? "completed" : ""}">
      ...
      <button class="icon-delete" onclick="removeRow('todo_items','${item.id}','todo')" aria-label="Delete">×</button>
    </div>
  `).join("") || `<div class="empty-state">Nothing to do right now ✨</div>`;
}

async function toggleTodo(id,completed){
  const {error}=await db.from("todo_items").update({completed}).eq("id",id);
  if(error) return alert(error.message);
  await loadTodos();
  await loadDashboard();
}
...
async function addGrocery(){
  try{
    const name=requireText("groceryName","Item name");
    const {error}=await db.from("grocery_items").insert({
      name,
      quantity:val("groceryQuantity"),
      category:val("groceryCategory"),
      purchased:false,
      created_by:currentUser.id
    });
    if(error) throw error;
    clear("groceryName","groceryQuantity");
    await loadGroceries();
    await loadDashboard();
  }catch(error){
    alert(error.message);
...
async function loadGroceries(){
  const {data,error}=await db.from("grocery_items")
    .select("*")
  ...
  document.getElementById("groceryList").innerHTML=(data||[]).map(item=>`
    <div class="check-item ${item.purchased ? "completed" : ""}">
      <label class="check-content">
        <input type="checkbox" ${item.purchased ? "checked" : ""}
          onchange="toggleGrocery('${item.id}',this.checked)">
        <span>
          <strong>${esc(item.name)}</strong>
          ${item.quantity ? `<small>${esc(item.quantity)}</small>` : ""}
          <em>${esc(item.category)}</em>
        </span>
      </label>
      <button class="icon-delete" onclick="removeRow('grocery_items','${item.id}','grocery')" aria-label="Delete">×</button>
    </div>
  `).join("") || `<div class="empty-state">The grocery list is empty 🛒</div>`;
}

async function toggleGrocery(id,purchased){
  const {error}=await db.from("grocery_items").update({purchased}).eq("id",id);
  if(error) return alert(error.message);
  await loadGroceries();
  await loadDashboard();
}
...
async function loadEntertainment(){
  ...
  if(entertainmentFilter!=="All"){
    query=query.eq("item_type",entertainmentFilter);
  }
  ...
  document.getElementById("entertainmentList").innerHTML=(data||[]).map(item=>`
    <div class="entertainment-card">
      <div class="entertainment-icon">${icons[item.item_type]||"✨"}</div>
      <div class="entertainment-body">
        <div class="spread">
          ...
          <button class="icon-delete" onclick="removeRow('entertainment_items','${item.id}','entertainment')">×</button>
        </div>
        ${item.description ? `<p>${esc(item.description)}</p>` : ""}
        <div class="rating-line">
          <span>Eliott: ${rating(item.rating_eliott)}</span>
          <span>Yasmin: ${rating(item.rating_yasmin)}</span>
        </div>
      </div>
    </div>
  `).join("") || `<div class="empty-state">Nothing in this category yet.</div>`;
}
